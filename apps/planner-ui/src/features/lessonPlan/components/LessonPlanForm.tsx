      ...
      <footer className="flex flex-col items-start gap-4 pt-4 border-t border-gray-100">
        {Object.keys(errors).length > 0 && (
          <div className="w-full bg-red-50 border border-red-100 text-red-600 p-4 rounded-lg text-sm font-bold flex items-center gap-3">
            ...
            אנא תקני את השגיאות בטופס לפני השמירה (שדות חסרים או לא תקינים).
          </div>
        )}
        <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center gap-2 bg-indigo-600 text-white px-10 py-3 rounded-lg font-bold shadow-lg hover:bg-indigo-700 disabled:opacity-50 transition-all active:scale-95 min-w-[200px]">
