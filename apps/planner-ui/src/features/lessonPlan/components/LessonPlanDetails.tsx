  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 print:hidden">
        <Link
          to="/"
          className="text-sm text-gray-500 hover:text-indigo-600 flex items-center gap-1 self-start sm:self-auto"
        >
            ...
            >
              <ul className="grid grid-cols-1 gap-3">
                {plan.operativeGoals.map((goal, i) => (
                  <li
                    key={i}
                    className="flex gap-3 items-start bg-orange-50/50 p-3 rounded-lg border border-orange-100 text-gray-800 print:bg-transparent print:border-0 print:p-0 print:mb-2"
                  >
                ...
                >
                  <div className="grid grid-cols-1 gap-3">
                    {plan.attachments.map((file) => (
                      <div
                        key={file.id}
                        className="bg-white p-3 rounded-lg border border-slate-200 flex items-center justify-between gap-3 shadow-sm"
                      >
                        <div className="flex-1 overflow-hidden">
                          <p
                            className="font-semibold text-gray-800 text-sm truncate mb-0.5"
                            title={file.filename}
                          >
                            {file.filename}
                          </p>
                    ...
                    <li key={i}>
                      {ref.startsWith("http") ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-indigo-600 hover:underline text-sm break-all print:text-gray-900 print:no-underline"
                        >
                          {ref}
                        </a>
                      ) : (
