...
export const LessonPlanDetails = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500" dir="rtl">
      {/* --- Header --- */}
      <div className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Link to="/" className="text-sm text-gray-500 hover:text-indigo-600 flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          >
             <ul className="grid grid-cols-1 gap-3">
               {plan.operativeGoals.map((goal, i) => (
                 <li key={i} className="flex gap-3 items-start bg-orange-50/50 p-3 rounded-lg border border-orange-100 text-gray-800">
          ...
          >
            <div className="relative border-r-2 border-green-100 mr-3 space-y-8 pr-6">
              {plan.lessonFlow.map((step, idx) => (
                <div key={idx} className="relative">
                  <div className="absolute -right-[33px] top-1 h-4 w-4 rounded-full bg-green-500 ring-4 ring-green-100"></div>
                  ...
                  <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm text-gray-700 whitespace-pre-line leading-relaxed">
                    {step.description}
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
           ...
           {/* 3. Attachments (Moved here - מתחת לציוד נדרש) */}
             ...
             <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
               ...
               <ul className="space-y-2">
                 {plan.references.map((ref, i) => (
                    <li key={i}>
                      {ref.startsWith('http') ? (
                        <a href={ref} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline text-sm break-all">{ref}</a>
                      ) : (
