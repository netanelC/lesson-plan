...
import { type User, type RoleType, MIN_PASSWORD_LENGTH, ROLE_LABELS, ROLES } from "@repo/types";
...
export const UserManagement = () => {
  ...
  const resetPasswordMutation = useMutation({
    mutationFn: (data: { id: string; newPassword?: string }) => api.post(`/users/${data.id}/reset-password`, { newPassword: data.newPassword }),
    onSuccess: () => {
          ...
          <tbody className="divide-y divide-gray-200 min-h-[300px]">
            {users?.map((user) => (
              <tr key={user.id} className={!user.isActive ? "bg-gray-50" : ""}>
                <td className={`px-6 py-4 flex items-center gap-3 ${!user.isActive ? "opacity-60" : ""}`}>
                  <img
                    src={user.avatarUrl ?? ""}
                  ...
                  <div>
                    <div className="text-sm font-bold flex items-center gap-2">
                      {user.fullName}
                      {!user.isActive && (
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-700">מושעה</span>
                      )}
                    </div>
                    <div className="text-xs text-gray-500">{user.email}</div>
                  </div>
                </td>
                <td className={`px-6 py-4 ${!user.isActive ? "opacity-60" : ""}`}>
                  <select
                    value={user.role}
                    onChange={(e) =>
                      roleMutation.mutate({
                ...
                <td className={`px-6 py-4 text-sm text-gray-500 text-right ${!user.isActive ? "opacity-60" : ""}`} dir="ltr">
                      ...
                      {openMenuId === user.id && (
                        ...
                        >
                          ...
                          <button
                            ...
                            disabled={currentUser.role === "OWNER" && user.id === currentUser.id}
                          >
      ...
      {userToResetPassword && (
        <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" aria-hidden="true" onClick={() => setUserToResetPassword(null)}></div>
            <span className="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div className="inline-block px-4 pt-5 pb-4 overflow-hidden text-right align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6" dir="rtl">
              ...
              <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                <button
                  type="button"
                  disabled={!newPassword || newPassword.length < MIN_PASSWORD_LENGTH || resetPasswordMutation.isPending}
                  ...
                  onClick={() => resetPasswordMutation.mutate({ id: userToResetPassword.id, newPassword })}
                >
