...
export const UserManagement = () => {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <h1 className="text-2xl font-bold text-gray-900">ניהול משתמשים והרשאות</h1>
            ...
            {users?.map(user => (
              <tr key={user.id}>
                <td className="px-6 py-4 flex items-center gap-3">
                  <img src={user.avatarUrl || ''} className="h-8 w-8 rounded-full bg-gray-200" alt="" />
                  <div>
                ...
                <td className="px-6 py-4">
                  <select 
                    value={user.role}
                    onChange={(e) => mutation.mutate({ id: user.id, role: e.target.value as UserRole })}
